...
import { X, BookOpen, Clock, Loader2, Sparkles } from "lucide-react";
...
export function AddTaskModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 1. INSERT DATA INTO SPECIFIC SCHEMA TABLES
      ...
      // 2. TRIGGER RE-ORCHESTRATION
      // This is what forces Cayley to move out of the way
      setSyncStatus("Re-balancing revision slots...");
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-end md:items-center justify-center bg-slate-900/60 backdrop-blur-md p-0 md:p-4 text-slate-900 animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-xl rounded-t-[2.5rem] md:rounded-[2.5rem] shadow-2xl overflow-hidden animate-in slide-in-from-bottom md:zoom-in-95 duration-300 max-h-[95vh] overflow-y-auto">
        ...
        <div className="p-6 md:p-8 pb-4 flex items-center justify-between sticky top-0 bg-white z-10">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl md:text-2xl font-black italic tracking-tighter">New Objective</h2>
          </div>
        ...
        <div className="px-6 md:px-8 pb-10 space-y-5">
          ...
          <div className="space-y-4">
            ...
            {activeTab === "weekly" && (
              <div className="space-y-1.5">
                ...
                >
                  ...
                  <option value="homework">Set Routine Time</option>
                </select>
              </div>
            )}
            ...
            <div className="space-y-1.5">
              ...
              <input
                placeholder="e.g. Finish Calculus Essay"
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none placeholder:text-slate-300"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">
                  {activeTab === "weekly" ? "Assigned Day" : "Due Date"}
                </label>
                {activeTab === "weekly" ? (
                  <select value={day} onChange={(e) => setDay(e.target.value)} className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none">
                  ...
                  <input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none" />
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <input type="number" value={load} onChange={(e) => setLoad(e.target.value)} className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold text-center focus:ring-2 focus:ring-blue-500 outline-none" />
