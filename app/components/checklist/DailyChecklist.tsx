...
export default function DailyChecklist({ day, completions }: Props) {
  const sections = [
    ...
    { label: "Revision", items: day.revision.slots.map(s => ({ 
        id: s.examId, 
        name: s.label || `${s.subject} Revision`, 
        subject: s.subject, 
        ...
        <section key={section.label}>
          ...
          <div className="space-y-3">
            {section.items.map((item: any) => {
              ...
              return (
                <div key={itemKey} className={`flex items-center justify-between p-4 rounded-xl border transition-all ${isDone ? "opacity-50 bg-gray-50 border-gray-100" : "bg-white border-gray-200 shadow-sm hover:border-blue-200"}`}>
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <p className={`text-sm font-semibold ${isDone ? "line-through text-gray-400" : "text-gray-700"}`}>
                          {item.name}
                        </p>
                        {item.subject && !isDone && (
                      ...
                      <p className="text-[11px] text-gray-400 font-medium">{item.minutes} mins</p>
                    </div>
                  </div>
                  {item.dueDate && (
