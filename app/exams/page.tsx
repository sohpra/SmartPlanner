...
import { syncRevisionSlots } from "@/lib/planner/revisionPersistence"; // 🎯 Import the orchestrator
import { ExamModal } from "../components/exams/ExamModal";
import { useExams } from "@/hooks/use-exams";
import { Trash2, Plus, AlertCircle, Target, Clock, Loader2 } from "lucide-react";

export default function ExamsPage() {
  const [open, setOpen] = useState(false);
  const { upcoming, loading, refresh } = useExams();
  const [hiddenIds, setHiddenIds] = useState<Set<string>>(new Set());
  const [isSyncing, setIsSyncing] = useState(false); // 🎯 Track the engine recalc
  ...
  async function deleteExam(id: string) {
    if (!confirm("Remove this exam? Other revision slots will be reshuffled.")) return;
    
    setIsSyncing(true);
    setHiddenIds((prev) => new Set(prev).add(id));

    try {
      // 🎯 1. First, kill any slots linked to this specific exam
      // This satisfies the Foreign Key constraint immediately.
      await supabase.from("revision_slots").delete().eq("exam_id", id);

      // 2. Now delete the exam
      const { error } = await supabase.from("exams").delete().eq("id", id);
      if (error) throw error;

      // 3. Trigger the Orchestrator to fill the newly created gaps
      await syncRevisionSlots();
      
      refresh();
    } catch (err: any) {
      alert("Error removing exam: " + err.message);
      // Rollback UI
      setHiddenIds((prev) => {
  ...
  return (
    <div className={`max-w-5xl mx-auto p-4 md:p-10 space-y-6 md:space-y-10 pb-24 animate-in fade-in duration-500 ${isSyncing ? "opacity-70 pointer-events-none" : ""}`}>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-gray-100 pb-6 md:pb-8">
        ...
        <div className="flex items-center gap-3">
          {isSyncing && (
            <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-blue-600 bg-blue-50 px-4 py-2 rounded-xl animate-pulse">
          ...
          {visibleExams.map((exam) => {
            ...
            return (
              <div key={exam.id} className="group relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between rounded-[1.5rem] md:rounded-[2rem] border-2 border-slate-100 bg-white p-5 md:p-7 shadow-sm hover:border-blue-500/20 transition-all">
                {/* ... (Keep your existing card JSX exactly as it is) ... */}
                <div className="flex flex-col gap-1 mb-4 md:mb-0">
                  ...
                  <div className="space-y-1">
                    <h3 className="text-xl md:text-2xl font-black italic tracking-tight text-slate-900 leading-tight">{exam.subject}</h3>
                ...
                <div className="flex flex-row md:flex-col items-center justify-between md:justify-center gap-4 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-8">
                  <div className="flex items-center gap-6 md:gap-8">
                    <div className="text-left md:text-right">
                      ...
                      <p className={`text-base md:text-lg font-black italic tracking-tight ${daysLeft <= 7 ? 'text-red-600' : 'text-slate-900'}`}>{daysLeft}d</p>
                    </div>
                    <div className="text-left md:text-right">
                      <p className="text-[7px] md:text-[8px] font-black text-slate-400 uppercase tracking-widest">Status</p>
                      <p className="text-base md:text-lg font-black italic tracking-tight text-blue-600">{exam.preparedness}%</p>
                    </div>
                  </div>
                  <button onClick={() => deleteExam(exam.id)} className="p-2 text-slate-200 hover:text-red-500 hover:bg-red-50 rounded-full transition-all"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            );
          })}
      ...
      <ExamModal 
        ...
        onAdded={async () => { 
          ...
          await syncRevisionSlots(); // 🎯 Sync after adding a new exam too
          await refresh(); 
          setIsSyncing(false);
        }} 
      />
